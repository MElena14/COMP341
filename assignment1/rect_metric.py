...
def compare_grasps(rect1, rect2):
    ...
    return (abs(score) > 0.25) and (abs(r1.angle - r2.angle) > 30)

def find_accuracy(Y, allGrasps):
    return allGrasps.apply_(lambda x: compare_grasps(Y, x)).any()

def grasp_accuracy(Ys, allGrasps, average=True):
    scores = all_grasp_scores(Ys, allGrasps)
    if not average:
        return sum(scores) / len(scores)
    return scores

def all_grasp_scores(Ys, allGrasps):
    return [find_accuracy(Ys[i], allGrasps[i]) for i in range(len(Ys))]
